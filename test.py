...
def plot(data, args):
    ...
    for i in range(len(args.sigma_train)):
        plt.plot(list(zip(*data[i]))[0], list(zip(*data[i]))[1], label=i/10)
...
def get_args():
    ...
    parser.add_argument('--sigma_train', type=list, default=[0.0, 0.5, 1.0,
                                                    1.5, 2.0
                                                    ])
